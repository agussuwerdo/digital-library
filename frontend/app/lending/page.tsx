...
export default function LendingPage() {
  ...
  const handleDelete = async (recordId: number) => {
    if (window.confirm('Are you sure you want to delete this lending record? This may affect analytics.')) {
      try {
            ...
            {records.length === 0 ? (
              ...
              records.map((record) => (
                <tr key={record.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.return_date ? formatDate(record.return_date) : 'Not Returned'}</td>
